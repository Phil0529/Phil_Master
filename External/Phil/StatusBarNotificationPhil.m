//
//  StatusBarNotificationPhil.m
//  PhilNotificationDemo
//
...
# pragma mark - PhilScrollLabel

@implementation PhilScrollLabel
{
...
# pragma mark - PhilWindowContainer

@implementation PhilWindowContainer
...
# pragma mark - PhilViewController

@interface PhilViewController()

@property (nonatomic, assign) NSInteger _PhilViewControllerSupportedInterfaceOrientation;

@end

@implementation PhilViewController
...
{
    self._PhilViewControllerSupportedInterfaceOrientation = supportedInterfaceOrientations;
}
...
typedef void(^PhilDelayedBlockHandle)(BOOL cancel);

static PhilDelayedBlockHandle perform_block_after_delay(CGFloat seconds, dispatch_block_t block)
{
	if (block == nil) {
		return nil;
	}
	...
	__block PhilDelayedBlockHandle delayHandleCopy = nil;

	PhilDelayedBlockHandle delayHandle = ^(BOOL cancel){
...
static void cancel_delayed_block(PhilDelayedBlockHandle delayedHandle)
{
...
# pragma mark - StatusBarNotificationPhil

@interface StatusBarNotificationPhil()

@property (strong, nonatomic) UITapGestureRecognizer *tapGestureRecognizer;
@property (strong, nonatomic) PhilDelayedBlockHandle dismissHandle;
@property (assign, nonatomic) BOOL isCustomView;

@end

@implementation StatusBarNotificationPhil
...
+ (StatusBarNotificationPhil *)shareInstance{
    static StatusBarNotificationPhil *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[StatusBarNotificationPhil alloc] init];
    });
    return instance;
}

- (StatusBarNotificationPhil *)init
{
    self = [super init];
    if (self) {
        // set default
        ...
        self.notificationStyle = PhilNotificationStyleStatusBarNotification;
        self.notificationAnimationInStyle = PhilNotificationAnimationStyleTop;
        self.notificationAnimationOutStyle = PhilNotificationAnimationStyleTop;
        self.notificationAnimationType = PhilNotificationAnimationTypeOverlay;
...
- (CGFloat)getNotificationLabelHeight
{
    switch (self.notificationStyle) {
        case PhilNotificationStyleStatusBarNotification:
            return [self getStatusBarHeight];
        case PhilNotificationStyleNavigationBarNotification:
...
- (void)setupNotificationView:(UIView *)view
{
    ...
    switch (self.notificationAnimationInStyle) {
        case PhilNotificationAnimationStyleTop:
            view.frame = [self getNotificationLabelTopFrame];
            break;
        case PhilNotificationAnimationStyleBottom:
            view.frame = [self getNotificationLabelBottomFrame];
            break;
        case PhilNotificationAnimationStyleLeft:
            view.frame = [self getNotificationLabelLeftFrame];
            break;
        case PhilNotificationAnimationStyleRight:
...
{
    self.notificationLabel = [PhilScrollLabel new];
...
{
    self.notificationWindow = [[PhilWindowContainer alloc] initWithFrame:[[[UIApplication sharedApplication] keyWindow] bounds]];
    ...
    PhilViewController *rootViewController = [[PhilViewController alloc] init];
...
- (void)createStatusBarView
{
    ...
    if (self.notificationAnimationType == PhilNotificationAnimationTypeReplace) {
...
- (void)displayNotificationWithViewPhil:(UIView *)view completion:(void (^)(void))completion
{
    if (!self.notificationIsShowing) {
...
{
    [self displayNotificationWithViewPhil:view completion:^{
        self.dismissHandle = perform_block_after_delay(duration, ^{
            [self dismissNotification];
        });
    }];
