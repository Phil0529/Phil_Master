 ...
 */
typedef void(^PhilCompletionBlock)(void);
 ...
 * label.
 */
@interface PhilScrollLabel : UILabel
/**
 ...
 */
@interface PhilWindowContainer : UIWindow
 ...
 */
@interface PhilViewController : UIViewController
 ...
 * notifications.
 */
@interface StatusBarNotificationPhil : NSObject

# pragma mark - enums

/**
 * @typedef PhilNotificationStyle
 * @brief Determines the notification style.
 */
typedef NS_ENUM(NSInteger, PhilNotificationStyle) {
    /// Covers the status bar portion of the screen.
    PhilNotificationStyleStatusBarNotification,
    /// Covers the status bar and navigation bar portions of the screen.
    PhilNotificationStyleNavigationBarNotification
};

/**
 * @typedef PhilNotificationAnimationStyle
 * @brief Determines the direction of animation for the notification.
 */
typedef NS_ENUM(NSInteger, PhilNotificationAnimationStyle) {
    /// Animate in from the top or animate out to the top.
    PhilNotificationAnimationStyleTop,
    /// Animate in from the bottom or animate out to the bottom.
    PhilNotificationAnimationStyleBottom,
    /// Animate in from the left or animate out to the left.
    PhilNotificationAnimationStyleLeft,
    /// Animate in from the right or animate out to the right.
    PhilNotificationAnimationStyleRight
};

/**
 * @typedef PhilNotificationAnimationType
 * @brief Determines whether the notification moves the existing content out of
 * the way or simply overlays it.
 */
typedef NS_ENUM(NSInteger, PhilNotificationAnimationType) {
    /// Moves existing content out of the way.
    PhilNotificationAnimationTypeReplace,
    /// Overlays existing content.
    PhilNotificationAnimationTypeOverlay
};

# pragma mark - properties

/// The label that holds the notification text.
@property (strong, nonatomic) PhilScrollLabel *notificationLabel;
...
@property (copy, nonatomic) PhilCompletionBlock notificationTappedBlock;
...
@property (strong, nonatomic) PhilWindowContainer *notificationWindow;
 ...
 * @c PhilNotificationStyleStatusBarNotification.
 */
@property (nonatomic) PhilNotificationStyle notificationStyle;
/**
 * Determines the direction from which the notification animates in. Default
 * value is @c PhilNotificationAnimationStyleBottom.
 */
@property (nonatomic) PhilNotificationAnimationStyle notificationAnimationInStyle;
/**
 * Determines the direction from which the notification animates out. Default
 * value is @c PhilNotificationAnimationStyleBottom.
 */
@property (nonatomic) PhilNotificationAnimationStyle
 ...
 * @c PhilNotificationAnimationTypeReplace.
 */
@property (nonatomic) PhilNotificationAnimationType notificationAnimationType;
/**
...
- (void)displayNotificationWithViewPhil:(UIView *)view
                             completion:(void (^)(void))completion;
