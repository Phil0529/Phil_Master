...
NSString* const remoteControlTogglePlayPauseButtonTapped = @"remoteControl_TogglePlayPause_pressed";
NSString* const remoteControlForwardButtonTapped = @"remoteControl_forward_pressed";
NSString* const remoteControlBackwardButtonTapped = @"remoteControl_backward_pressed";
NSString* const remoteControlOtherButtonTapped = @"remoteControl_other_pressed";
NSString* const StartGifVieRadioPhil = @"StartGifVieRadioPhil";
NSString* const StopGifVieRadioPhil = @"StopGifVieRadioPhil";
NSString* const PlayRadioStatus = @"Play_Radio_Status";
    ...
    if (audioSession.category != AVAudioSessionCategoryPlayback) {
        ...
        if ([device respondsToSelector:@selector(isMultitaskingSupported)]) {
            if (device.multitaskingSupported) {
                ...
                if (aError) {
                    if (!self.disableLogs) {
                        SWLogD(@"set category error:%@",[aError description]);
                    }
                }
                aError = nil;
                ...
                if (aError) {
                    if (!self.disableLogs) {
                        SWLogD(@"set active error:%@",[aError description]);
                    }
                }
        ...
        if (!self.disableLogs) {
            SWLogD(@"unable to register background playback");
        }
    }
...
- (void)setupPlayerItemWithUrl:(NSURL *)url index:(NSInteger)index
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        //        SWLogD(@"---------askdjalksdjalksdjalksjdlaskjdalskjdas----------%i", self.playerItems.count);
    });
...
{
    [self.audioPlayer play];
    [[NSNotificationCenter defaultCenter] postNotificationName:StartGifVieRadioPhil object:nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"radioPhil" object:@"0"];
    
    if ([EZBroadcastManager shareInstance].isDisplayRadioStatusBar) {
        [[EZBroadcastManager shareInstance] displayRadioStatusBar];
    }
...
{
    [self.audioPlayer pause];
    [[NSNotificationCenter defaultCenter] postNotificationName:StopGifVieRadioPhil object:nil];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"radioPhil" object:@"1"];
    [[EZBroadcastManager shareInstance].statusBarNotification dismissNotification];
}

- (NSInteger )playNext
{
    if (_shuffleMode == HysteriaPlayerShuffleModeRadom) {
        ...
        if (nextIndex != NSNotFound) {
            ...
            return [[self getHysteriaIndex:self.audioPlayer.currentItem] integerValue];
        }
    } else {
        ...
        if (nowIndex + 1 < [self hysteriaPlayerItemsCount]) {
            ...
            if (_repeatMode == HysteriaPlayerShuffleModeOn) {
                [self pausePlayerForcibly:YES];
                if ([self.delegate respondsToSelector:@selector(hysteriaPlayerDidReachEnd)]) {
                    [self.delegate hysteriaPlayerDidReachEnd];
                }
...
- (NSInteger )playPrevious
{
    ...
    if (nowIndex == 0)
    {
        if (_repeatMode == HysteriaPlayerRepeatModeOn) {
            ...
            return [self hysteriaPlayerItemsCount] - 1;
        } else {
...
- (BOOL)isPlaying
{
    return self.isInEmptySound ? NO : self.audioPlayer.rate != 0.f;
}
...
- (void)interruption:(NSNotification*)notification
{
    ...
    if (!self.disableLogs) {
        SWLogD(@"HysteriaPlayer interruption: %@", interuptionType == AVAudioSessionInterruptionTypeBegan ? @"began" : @"end");
    }
}
...
- (void)routeChange:(NSNotification *)notification
{
    ...
    if (!self.disableLogs) {
        SWLogD(@"HysteriaPlayer routeChanged: %@", routeChangeType == AVAudioSessionRouteChangeReasonNewDeviceAvailable ? @"New Device Available" : @"Old Device Unavailable");
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object
    ...
    if (object == self.audioPlayer && [keyPath isEqualToString:@"status"]) {
        if (self.audioPlayer.status == AVPlayerStatusReadyToPlay) {
            ...
            if (!self.disableLogs) {
                SWLogD(@"=---asdasdasdkja%@", self.audioPlayer.error);
            }
...
{
    if (!self.playerItems && isMemoryCached) {
