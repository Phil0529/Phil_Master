...
//    [preBtn setTitle:LBLocalized(@"preview") forState:UIControlStateNormal];
...
//{
//    if ([_delegate respondsToSelector:@selector(jkImagePickerControllerDidCancel:)]) {
//        [_delegate jkImagePickerControllerDidCancel:self];
//    }
//}
//
...
//    
//    if (minimumNumberOfSelection  <= self.maximumNumberOfSelection) {
//        return (numberOfSelections + self.imageCount <= self.maximumNumberOfSelection);
//    }
//    return YES;
//}
//
...
//        [cancelBtn setTitle:LBLocalized(@"cancel") forState:UIControlStateNormal];
...
//        cell.asset = nil;
//    }else{
//        ALAsset *asset = self.assetsArray[indexPath.item - 1];
//        cell.asset = asset;
...
//    
//    NSURL *assetURL = [asset valueForProperty:ALAssetPropertyAssetURL];
//    [self addAssetsObject:assetURL thumb:[UIImage imageWithCGImage:[asset thumbnail]] real:nil metaDict: representation.metadata imgData:nil];
...
//{
//    
//    if (self.selectedAssetArray.count + self.imageCount >= self.maximumNumberOfSelection) {
//        NSString  *str = [NSString stringWithFormat:@"%@%lu%@",LBLocalized(@"Most choose"),(unsigned long)self.maximumNumberOfSelection,LBLocalized(@"pictures")];
...
//        ALAssetRepresentation* representation = [assetsCell.asset defaultRepresentation];
//        UIImage *thumbImg = [UIImage imageWithCGImage:[assetsCell.asset thumbnail]];
//        [self addAssetsObject:assetURL thumb:thumbImg real:nil metaDict: representation.metadata imgData:nil];
//        [self resetFinishFrame];
...
//}
//
//- (void)addAssetsObject:(NSURL *)assetURL thumb:(UIImage *)thumb real:(UIImage *)real metaDict:(NSDictionary *)metaDict imgData:(NSData *)imgData;
//{
//    @autoreleasepool {
...
//    asset.thumbImg = thumb;
//    asset.metaDict = metaDict;
...
//    self.finishLabel.text = [NSString stringWithFormat:@"%@(%ld)",LBLocalized(@"done") ,(unsigned long)self.selectedAssetArray.count];
...
//        [_titleButton.titleLabel setFont:[UIFont systemFontOfSize:16.5f]];
...
//        _titleLabel.font = [UIFont systemFontOfSize:17.5f];
...
//        [_finishButton setBackgroundColor:FOREGROUND_COLOR];
//        [_finishButton.layer setCornerRadius:3.f];
//        [_finishButton setClipsToBounds:YES];
